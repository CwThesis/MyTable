...
const hasStripe: Ref<boolean> = ref(false);
      ...
      <div id="body" class="p-2">
        <div class="flex flex-col justify-center rounded-lg m-1">
          <div v-if="banner" class="relative">
            ...
            <h1
              class="absolute text-6xl text-center text-white top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 font-josefin">
              {{ heading }}</h1>
          </div>
        ...
        <div v-if="orderStore.currentOrder && hasStripe"
          class="z-10 flex fixed bottom-0 w-full bg-transparent h-20 p-2 justify-center items-center">
          <button
            class="bg-violet-700 text-white font-semibold w-5/6 h-5/6 rounded-lg flex flex-row items-center justify-center"
            @click="sendOrder">
            <p class="font-normal text-lg"> ORDER for {{ orderStore.currentTotal }} EUR</p>
          </button>
        </div>
        <div v-else-if="orderStore.currentOrder && !hasStripe">
        Oops, looks like the restaurant has not enabled payments yet
        </div>
      <div
        class="flex flex-col fixed h-20 top-0 bg-transparent pt-2 hover:font-bold text-violet-700  font-semibold w-full items-center justify-between rounded pl-4">
        <button class="bg-violet-700 m-4 hover:bg-violet-500 text-white font-semibold self-end w-1/5 rounded p-2 mr-5"
          @click="seeTicket">
